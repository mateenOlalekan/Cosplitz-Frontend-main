...
const AllSplitsPage = () => {
  ...
  const { splits, isLoading, loadSplits } = useSplitStore();
  ...
  const filteredSplits = useMemo(() => {
    return splits.filter((split) => {
      const title = split.title?.toLowerCase() || '';
      const description = split.description?.toLowerCase() || '';
      const location = split.location?.toLowerCase() || '';
      ...
      const matchesCategory =
        selectedCategory === 'all' || split.category === selectedCategory;

      return matchesSearch && matchesCategory;
    });
  }, [splits, searchTerm, selectedCategory]);
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-gray-600 mt-1">
                Join splits created by others or create your own
              </p>
            </div>
            <button
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border">
              ...
              <p className="text-3xl font-bold text-green-600 mt-2">
                {splits.filter(
                  (s) => (s.current_participants || 0) < s.max_participants
                ).length}
              </p>
            </div>
            <div className="bg-white p-6 rounded-xl shadow-sm border">
